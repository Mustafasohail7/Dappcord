const Channels = ({ provider, account, dappcord, channels, currentChannel, setCurrentChannel }) => {
  ...
  const channelHandler = async(channel) => {
    const joined = await dappcord.hasJoined(channel.id,account)
    if(joined){
      ...
      <div className="channels__text">
        ...
        <ul>
          {channels.map((channel,index) => (
             ...
             className={currentChannel && currentChannel.id.toString() === channel.id.toString() ? "active" : ""}>{channel.name}</li>
          ))}
        </ul>
